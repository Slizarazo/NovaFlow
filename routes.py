...
def create_educacion():
    try:
        ...
        # Imprimir el JSON completo recibido
        print("=" * 60)
        print("📥 DATOS JSON RECIBIDOS EN /api/educacion")
        print("=" * 60)
        print(json.dumps(data, indent=2, ensure_ascii=False))
        print("=" * 60)
        ...
        # Extraer los datos del formulario de educación
        ...
        if not titulo or not institucion:
            return jsonify({'status': 'error', 'message': 'Título e institución son campos obligatorios'}), 400
        ...
        app.logger.error(f"Error al crear educación: {str(e)}")
        return jsonify({'status': 'error', 'message': str(e)}), 500
...
def create_certificacion():
    try:
        ...
        # Imprimir el JSON completo recibido
        print("=" * 60)
        print("📥 DATOS JSON RECIBIDOS EN /api/certificaciones")
        print("=" * 60)
        print(json.dumps(data, indent=2, ensure_ascii=False))
        print("=" * 60)
        ...
        # Extraer los datos del formulario de certificación
        nombre_certificacion = data.get('nombre_certificacion')
        emisor = data.get('emisor')
        fecha_emision = data.get('fecha_emision')
        fecha_vencimiento = data.get('fecha_vencimiento')
        credencial_id = data.get('credencial_id')
        url_verificacion = data.get('url_verificacion')

        print("🏆 DATOS PROCESADOS:")
        print(f"  - Nombre: {nombre_certificacion}")
        print(f"  - Emisor: {emisor}")
        print(f"  - Fecha Emisión: {fecha_emision}")
        print(f"  - Fecha Vencimiento: {fecha_vencimiento}")
        print(f"  - ID Credencial: {credencial_id}")
        print(f"  - URL Verificación: {url_verificacion}")

        # Validaciones básicas
        if not nombre_certificacion or not emisor:
            return jsonify({'status': 'error', 'message': 'Nombre de certificación y emisor son campos obligatorios'}), 400
        ...
        app.logger.error(f"Error al crear certificación: {str(e)}")
        return jsonify({'status': 'error', 'message': str(e)}), 500

@app.route('/api/proyectos-destacados', methods=['POST'])
@login_required
def create_proyecto_destacado():
    try:
        data = request.get_json()
        
        # Imprimir el JSON completo recibido
        print("=" * 60)
        print("📥 DATOS JSON RECIBIDOS EN /api/proyectos-destacados")
        print("=" * 60)
        print(json.dumps(data, indent=2, ensure_ascii=False))
        print("=" * 60)
        ...
        # Extraer los datos del formulario de proyecto
        ...
        descripcion = data.get('descripcion')
        url_proyecto = data.get('url_proyecto')
        ...
        print(f"  - Fecha Inicio: {fecha_inicio}")
        print(f"  - Fecha Fin: {fecha_fin}")
        print(f"  - Rol: {rol_proyecto}")
        print(f"  - Tecnologías: {tecnologias}")
        print(f"  - Descripción: {descripcion}")
        print(f"  - URL: {url_proyecto}")

        # Validaciones básicas
        if not nombre_proyecto or not cliente:
            return jsonify({'status': 'error', 'message': 'Nombre del proyecto y cliente son campos obligatorios'}), 400

        return jsonify({
            'status': 'success', 
            'message': 'Proyecto destacado agregado exitosamente',
            'data': {
        ...
        app.logger.error(f"Error al crear proyecto destacado: {str(e)}")
        return jsonify({'status': 'error', 'message': str(e)}), 500

@app.route('/api/habilidades-tecnicas', methods=['POST'])
@login_required
def create_habilidad_tecnica():
    try:
        data = request.get_json()
        
        # Imprimir el JSON completo recibido
        print("=" * 60)
        print("📥 DATOS JSON RECIBIDOS EN /api/habilidades-tecnicas")
        print("=" * 60)
        print(json.dumps(data, indent=2, ensure_ascii=False))
        print("=" * 60)
        ...
        # Extraer los datos del formulario de habilidad técnica
        nombre_habilidad = data.get('nombre_habilidad')
        categoria_habilidad = data.get('categoria_habilidad')
        nivel_habilidad = data.get('nivel_habilidad')
        experiencia_anos = data.get('experiencia_anos')
        descripcion = data.get('descripcion')

        print("🛠️ DATOS PROCESADOS:")
        print(f"  - Nombre Habilidad: {nombre_habilidad}")
        print(f"  - Categoría: {categoria_habilidad}")
        print(f"  - Nivel: {nivel_habilidad}%")
        print(f"  - Años Experiencia: {experiencia_anos}")
        print(f"  - Descripción: {descripcion}")

        # Validaciones básicas
        if not nombre_habilidad or not nivel_habilidad:
            return jsonify({'status': 'error', 'message': 'Nombre de habilidad y nivel son campos obligatorios'}), 400

        return jsonify({
            'status': 'success', 
            'message': 'Habilidad técnica agregada exitosamente',
            'data': {
        ...
        app.logger.error(f"Error al crear habilidad técnica: {str(e)}")
        return jsonify({'status': 'error', 'message': str(e)}), 500
...
def create_idioma():
    try:
        ...
        # Imprimir el JSON completo recibido
        print("=" * 60)
        print("📥 DATOS JSON RECIBIDOS EN /api/idiomas")
        print("=" * 60)
        print(json.dumps(data, indent=2, ensure_ascii=False))
        print("=" * 60)
        ...
        # Extraer los datos del formulario de idioma
        idioma = data.get('idioma')
        nivel_idioma = data.get('nivel_idioma')
        certificacion_idioma = data.get('certificacion_idioma')
        ...
        print(f"  - Nivel: {nivel_idioma}")
        print(f"  - Certificación: {certificacion_idioma}")
        print(f"  - Puntuación: {puntuacion}")
        print(f"  - Lectura: {lectura}")
        print(f"  - Escritura: {escritura}")
        print(f"  - Conversación: {conversacion}")
        print(f"  - Comprensión: {comprension}")

        # Validaciones básicas
        if not idioma or not nivel_idioma:
            return jsonify({'status': 'error', 'message': 'Idioma y nivel son campos obligatorios'}), 400
        ...
        return jsonify({
            ...
            'data': {
                ...
                'certificacion_idioma': certificacion_idioma,
        ...
        app.logger.error(f"Error al crear idioma: {str(e)}")
        return jsonify({'status': 'error', 'message': str(e)}), 500
